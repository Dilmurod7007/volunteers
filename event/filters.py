...
#         if value:
#             lt = split_string(value)
#             queryset = Event.objects.filter(position_category__images_for_positioncategory__colored_images__color_title__in=lt)
#             return queryset
#         else:
#             return queryset
...
class EventFilter(django_filters.FilterSet):
    starting_date = django_filters.DateFilter(lookup_expr="gte")
    finishing_date = django_filters.DateFilter(lookup_expr="lte")
    direction = CharFilter(method='get_direction')
    ...
    def get_direction(self, queryset, name, value):
        if value:
            lt = split_string(value)
            return queryset.filter(direction__title__in=lt)
        else:
            return queryset            
